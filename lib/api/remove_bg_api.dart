...
class ApiClient {
  Future<Uint8List> removeBgApi(String imagePath) async {
    // Future<Uint8List> removeBgApi(Uint8List imagePath) async {
    var request = http.MultipartRequest(
        "POST", Uri.parse("https://api.remove.bg/v1.0/removebg"));
    request.files
        .add(await http.MultipartFile.fromPath("image_file", imagePath));
    // .add(http.MultipartFile.fromBytes("image_file", imagePath));
    request.headers.addAll(
        {"X-API-Key": "8m1hy4MAfugvzX5itNJ1JAC3"}); //Put Your API key HERE
